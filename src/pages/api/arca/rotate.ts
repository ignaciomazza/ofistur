...
export default async function handler(
  ...
  try {
    ...
    logArca("error", "API rotate error", { error: err instanceof Error ? err.message : "Error inesperado" });
    return res.status(503).json({ error: "No se pudo iniciar la reconexión ARCA. Volvé a intentar en unos minutos." });
  }
}
